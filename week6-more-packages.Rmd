...
If/when you want to go into R packaging in more depth, see Hadley Wickham's book [R Packages](http://r-pkgs.had.co.nz/). Another new resource is this [R package workshop material](https://combine-australia.github.io/r-pkg-dev/).

## Checking your package

Just use the RStudio Check in the Build tab.
...
RStudio > File > New > R Markdown > From Template > Vignette
...
Just use knit button.

### Getting your vignette to show up

This is a disaster currently in RStudio. They'll probably fix eventually.

Set the build options to make the vignettes. Tools > Project Options > Build Options

But then when you Install and Restart, your vignettes don't show up.

Supposedly this works:

* Build to tar.gz
* `install.packages("..location../MyNewPackage_0.1.0.tar.gz", repos = NULL, type = "source")`
* `browseVignettes("MyNewPackage")` should work... Works on my PC and not on my Mac.
...
When ready to push to GitHub or GitLab,

```
devtools::build_vignettes()
```

Creates `doc` and `Meta` at base. Move `doc` to `inst` folder and delete `Meta`.

Now RStudio, Install and Restart works and build from source works on GitHub.

**Careful, RStudio `Check` will trash the `inst/doc` folder and not recreate it. So always check that `inst/doc` not been trashed.
